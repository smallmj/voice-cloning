...
m = load_model("mlx-community/chatterbox-multilingual-v3")
...
for i,t in enumerate(texts):
    t0=time.perf_counter()
    res=list(m.generate(t, ref_audio="ref.wav", lang_code="zh"))
    el=time.perf_counter()-t0
    a=np.concatenate([r.audio for r in res]) if len(res)>1 else res[0].audio
    dur=len(a)/24000; ta+=dur; ts+=el
    sf.write(f"cbx{i}.wav", np.array(a), 24000)

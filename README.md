...
- **语气/非语言标记**：按引擎原生语法呈现（VoxCPM2 标签、MiniMax 语气词与停顿标记），带搜索分类下拉，不支持的引擎自动隐藏；文本归一化保护标记语法。
- **指令控制**：Qwen3-TTS 云端 instruct 引擎用自然语言指令控制语气/语速/方言。
- **对比与调优**：跨引擎盲听对比（LUFS 归一化）、偏好画像；文本归一化层与归一化预览。
- **数据管理**：音色包导出导入、整库备份恢复；存储层 SQLite + 一次性迁移 + 快照备份。
- **多主题界面**：深色 / 浅色 / 跟随系统三态主题，侧栏开关移至顶栏右侧。

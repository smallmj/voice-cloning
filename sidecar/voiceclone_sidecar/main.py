...
def create_app(
    ...
    # 中文回归会话 + 固化能力矩阵（issue #16）：又一个持久索引。
    regression_store = RegressionStore(data_root)
                    ...
                    if audio_file:
                        ...
                        if asr_check:
                            try:
                                transcript = await asyncio.get_running_loop().run_in_executor(
                                    None,
                                    lambda: _local_transcriber().transcribe(
    ...
    @app.get("/regression/sessions", dependencies=[Depends(require_auth)])
    ...
    @app.get("/regression/{session_id}", dependencies=[Depends(require_auth)])
    ...
    @app.delete("/regression/{session_id}", dependencies=[Depends(require_auth)])
    ...
    @app.get("/capability-matrix", dependencies=[Depends(require_auth)])

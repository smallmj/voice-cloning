...
function CapabilityMatrixSection({
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch(`${baseUrl}/capability-matrix`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        if (res.ok) setMatrix((await res.json()) as CapabilityMatrix);
      } catch {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        const res = await fetch(`${baseUrl}/regression/${regression.id}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        if (res.ok) setRegression(await res.json());
      } catch {
        /* keep polling */
      }
    }, 1500);
  ...
  async function runRegression() {
    ...
    setError(null);
    try {
      const res = await fetch(`${baseUrl}/regression/run`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
        body: JSON.stringify({}),
      });
      ...
      if (!res.ok) {
        setError(data?.detail ?? `启动回归失败（HTTP ${res.status}）`);
        return;
      }
      setRegression(data);
    } catch (err) {
  ...
  return (
    <section>
      ...
      <div className="hint">
        主观听感（盲听 MOS）请用上方「盲听对比」：同音色同文本、LUFS 归一化、盲标签评分；
